...
describe('live Markdown styles', () => {
  ...
  it('keeps inline styles paint-only and restores level/block styles separately', () => {
    ...
    expect(styles).toMatch(/\.dsh-better-composer-expand-button\s*\{/u)
    expect(styles).toMatch(/\.dsh-better-composer-toast\s*\{/u)
    ...
    expect(styles).toMatch(/\.dsh-better-composer-table-separator\s*\{[^}]*height:\s*0/u)
    expect(styles).toMatch(/\.dsh-better-composer-visual-layer\s*\{[^}]*position:\s*absolute/u)
    expect(styles).toMatch(/\[data-composer-input\]\[data-better-composer-visual-root\]\s*\{[^}]*-webkit-text-fill-color:/u)
    ...
    expect(styles).toMatch(/\.dsh-better-composer-ctxmap-donut\s*\{/u)
    expect(styles).toMatch(/\.dsh-better-composer-ctxmap-bars-axis\s*\{/u)
    expect(styles).toMatch(/\.dsh-better-composer-ctxmap-bar-anchor\s*\{/u)
    expect(styles).toMatch(/prefers-reduced-motion:\s*reduce/u)
  })
